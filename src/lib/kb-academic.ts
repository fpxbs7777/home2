 ...
 *
 *  El índice vive en public/kb/academic-index.json (vectores Float16 base64 +
...
async function cargarIndice(baseUrl?: string): Promise<{
  ...
  // 1) fetch desde la raíz pública (usa baseUrl si está disponible; si no, ruta relativa)
  try {
    const url = baseUrl
  ...
  // 2) fallback Node (dev): leer desde public/ del proyecto
  if (!json) {
    try {
      const fs = await import("node:fs");
      const candidatos = [
        "public/kb/academic-index.json",
        "kb/academic-index.json",
        "src/kb/academic-index.json",
      ];
      for (const c of candidatos) {
        if (fs.existsSync(c)) {
          json = JSON.parse(fs.readFileSync(c, "utf8")) as IndiceAcademico;
          break;
        }
      }
    } catch {
